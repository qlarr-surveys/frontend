// Post-move tests - imports from new locations (stateUtils.js and addInstructions.js)
// Same test cases as baseline — identical results prove the move is faithful
...
import { cleanupRandomRules } from '../addInstructions.js';
...
describe('cleanupRandomRules (moved to addInstructions)', () => {
  test('calls without throwing for randomize_questions', () => {
    const component = {
      randomize_questions: 'YES',
      children: [{ code: 'G1' }, { code: 'G2' }],
      instructionList: [
        { code: 'random_group', groups: [{ codes: ['G1', 'G2'], randomOption: 'YES' }] },
      ],
    };
    expect(() => cleanupRandomRules(component)).not.toThrow();
  });
});
